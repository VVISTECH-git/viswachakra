// Fill the SETTLED amount for PAID claims that are missing it: deep-scrape each paid
// case's workflow, push it to Supabase, and set paid_amount / deduction / settlement_days
// / paid_date / is_paid from the "paid" row. Does NOT touch claimed_amount or list fields.
// Crash-safe (per case). Same engine as refresh-unpaid.js, different target set.
//
// Usage: node refresh-paid.js [--limit N]
...
async function fetchPaidMissing() {
  const out = []; let f = 0;
  while (true) { const { data, error } = await s.from('cases').select('case_no,claim_status,is_paid,paid_amount').range(f, f + 999); if (error) throw new Error(error.message); if (!data || !data.length) break; out.push(...data); if (data.length < 1000) break; f += 1000; }
  return out.filter(r => (r.is_paid || paidRe.test(r.claim_status || '')) && r.paid_amount == null).map(r => r.case_no);
}
...
(async () => {
  ...
  const doneFile = path.join(__dirname, 'logs', 'refresh-paid-done.txt');
  const doneSet = new Set(fs.existsSync(doneFile) ? fs.readFileSync(doneFile, 'utf8').split(/\r?\n/).map(l => l.trim()).filter(Boolean) : []);
  let caseNos = (await fetchPaidMissing()).filter(c => !doneSet.has(c));
  if (limit > 0) caseNos = caseNos.slice(0, limit);
  console.log(`Paid cases missing settled amount: ${caseNos.length} (skipping ${doneSet.size} already done)`);

  const logFile = path.join(__dirname, 'logs', 'refresh-paid.log');
  fs.appendFileSync(logFile, `\nrefresh-paid resumed ${new Date().toISOString()} (${caseNos.length} remaining)\n`);
  ...
  console.log(`\nDone. ${done - failed} paid cases got settled amount, ${failed} failed.`);
  if (failed) console.log('Re-run to retry failed cases (safe - upserts).');
